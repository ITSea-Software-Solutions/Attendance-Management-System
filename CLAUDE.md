...
## Feature Status (as of 2026-08-31 — web v1.19.0, apps v0.9.37)
...
- [x] **v1.19.0: manual attendance + payroll corrections** — `manual_attendance_requests` (migration 044) + `ManualAttendanceRequest`; `AttendanceController@manualStore/manualRequests/decideManual`, routes `POST /attendance/manual`, `GET /attendance/manual-requests`, `POST /attendance/manual-requests/{id}/decide`. Company admin/HR/super enter a missed day directly (row written auto-approved, logs created immediately); `vendor_admin` raises a request and **no attendance_logs row exists until approval**. Guards: approved deployment must cover `work_date`; refuses a date that already has logs (re-checked at decide time — a gate mark while pending rejects the request); `reason` mandatory; `method=manual` + `override_reason` on both logs; OUT before IN rolls to next day (night shift). **Model casts use explicit formats (`date:Y-m-d`, `datetime:Y-m-d H:i`)** — the default serializer emits UTC and shifted an 08:30 IST entry to the previous day. UI: `frontend/src/components/ManualAttendance.jsx` on AttendanceList AND PayrollWages (approvals panel + "Add a missed day" form; accepts `prefillWorkerId` so a per-row button opens it for that worker). **Payroll is now scoped per contractor**: `scope()` + `PayrollService` accept `vendor_ids` (one or many, CSV or array; singular `vendor_id` kept working; a contractor login's own `vendor_id` still wins so the param cannot widen their scope) — verified across register/register-export/muster/contractor-summary; Contractors MultiSelect added to Payroll.jsx (reads `?vendor_ids=` from the URL) and payroll actions added to PayrollWages.jsx. Payroll fixes: register rate box wrote `monthly_rate` for every worker (daily workers are paid from `daily_rate`, so it saved and changed nothing) — now branches on `wage_type`; WorkerRegister's Employment tab never hydrated from `existingWorker`; "Daily wage"/"Monthly salary" wording; muster available as Excel. **`xlsx` moved off npm's abandoned 0.18.5 to SheetJS's official 0.20.3** (prototype pollution CVSS 7.8, reachable via ImportWorkersModal parsing user files).
